...
@dataclass
class PolarizerModel:
    """Non-ideal linear polarizer model that supports finite exctintion ratio."""

    extinction_ratio: float = np.inf

    @property
    def polarization_efficiency(self) -> float:
        """Compute degree of polarization produced by the filter"""
        if np.isinf(self.extinction_ratio):
            return 1.0

        er = self.extinction_ratio

        if er <= 1:
            raise ValueError(
                "Extinction ratio must be > 1"
            )

        return (er - 1.0) / (er + 1.0)

    def stokes(self, theta_rad: float) -> NDArray:
        """Compute the Stokes state generated by the filter, assuming unit intensity before the polarizer."""

        p = self.polarization_efficiency

        return np.array([
            1.0,
            p * np.cos(2.0 * theta_rad),
            p * np.sin(2.0 * theta_rad),
        ], dtype=np.float64)
...
class CalibrationInput:
    ...
    polarization_model: PolarizerModel
...
class CalibrationConstructor:
    ...
    def __init__(self, inp_data: CalibrationInput) -> None:
        ...
        self.polarization_model = inp_data.polarization_model
    ...
    def _construct_matrices(self) -> None:
        ...
        for angle, data in self.angle_data.items():
            ...
            stokes = self.polarization_model.stokes(theta)
    ...
    def compute_calibration(self) -> Calibration:
        ...
        A += 1e-8 * np.eye(4)[None, :, :]  # Regularization
